...
class LMStudioNativeClient:
    """Client for LM Studio's native REST API (POST /api/v1/chat).

    Unlike the OpenAI-compatible endpoint, the native API accepts a
    ``reasoning`` request field, letting us force ``"off"`` for models such
    as qwen3 that would otherwise spend their whole token budget thinking.
    Accepts either the server base URL (http://127.0.0.1:1234) or its
    OpenAI-compatible form with a trailing /v1; both are normalized.
    """
